...
const ProjectsTab = ({ 
  ...
  useEffect(() => {
    if (!projects.length && !isLoading.projects) {
      onFetchData('projects');
    }
  }, [projects.length, isLoading.projects, onFetchData]);
  ...
  if (projects.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-secondary text-lg mb-4">No projects found</div>
      </div>
    );
  }
    ...
    <div className="flex gap-3">
      {project.image && (
        <div className="flex-shrink-0">
          <img 
            src={getImagePath(project.image, 'project')} 
            alt={project.name} 
